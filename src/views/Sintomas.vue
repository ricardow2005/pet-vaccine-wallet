<template>
  <v-container>
    <v-toolbar flat color="white">
      <v-toolbar-title>Lista de Sintomas</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn color="primary" dark class="mb-2" @click="newCliente()">Novo Sintomas</v-btn>
    </v-toolbar>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Novo Sintomas" : "Editar Sintomas";
    }
  },
  ...
  methods: {
    ...
    deleteItem(item) {
      console.log(item);
      axios
        .delete(this.backend + "/sintomasdelete/", {
          data: { id: item.id }
        })
        .then(response => {
    ...
    saveCliente() {
      if (this.editedIndex == -1) {
        ...
        axios
          .post(this.backend + "/sintomascreate/", this.editedItem)
          .then(response => {
            this.getData();
    ...
    updateCliente() {
        ...
        axios
          .put(
            this.backend + "/sintomasupdate/" + this.editedItem.id + "/",
            this.editedItem
          )
          .then(response => {
            this.getData();
    ...
    getData() {
      ...
      axios.get(this.backend + "/sintomas/").then(response => {
