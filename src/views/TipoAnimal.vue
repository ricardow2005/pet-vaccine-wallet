<template>
  <v-container>
    <v-toolbar flat color="white">
      <v-toolbar-title>Lista de Tipos de Animais</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn color="primary" dark class="mb-2" @click="newCliente()">Novo Tipo de Animal</v-btn>
    </v-toolbar>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Novo Tipo de Animal" : "Editar Tipo de Animal";
    }
  },
  ...
  methods: {
    ...
    deleteItem(item) {
      console.log(item);
      axios
        .delete(this.backend + "/tipoanimaldelete/", {
          data: { id: item.id }
        })
        .then(response => {
    ...
    saveCliente() {
      if (this.editedIndex == -1) {
        ...
        axios
          .post(this.backend + "/tipoanimalcreate/", this.editedItem)
          .then(response => {
            this.getData();
    ...
    updateCliente() {
        ...
        axios
          .put(
            this.backend + "/tipoanimalupdate/" + this.editedItem.id + "/",
            this.editedItem
          )
          .then(response => {
            this.getData();
