<template>
  <v-container>
    <v-toolbar flat color="white">
      <v-toolbar-title>Lista de Tipos de Medicamentos</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn color="primary" dark class="mb-2" @click="newCliente()">Novo Tipo de Medicamento</v-btn>
    </v-toolbar>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Novo Tipo de Medicamento" : "Editar Tipo de Medicamento";
    }
  },
  ...
  methods: {
    ...
    deleteItem(item) {
      console.log(item);
      axios
        .delete(this.backend + "/tipomedicamentodelete/", {
          data: { id: item.id }
        })
        .then(response => {
    ...
    saveCliente() {
      if (this.editedIndex == -1) {
        ...
        axios
          .post(this.backend + "/tipomedicamentocreate/", this.editedItem)
          .then(response => {
            this.getData();
    ...
    updateCliente() {
        ...
        axios
          .put(
            this.backend + "/tipomedicamentoupdate/" + this.editedItem.id + "/",
            this.editedItem
          )
          .then(response => {
            this.getData();
